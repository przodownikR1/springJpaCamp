...
== Moduły
[square]
** **http://mvnrepository.com/artifact/org.hibernate/hibernate-core[Hibernate Core]**
...
** **http://mvnrepository.com/artifact/org.hibernate/hibernate-annotations[Hibernate Annotation]**

***  dostarcza znacziki metadanych równorzędne do konfiguracji mapować w XML
***  redukcja XML

** **http://mvnrepository.com/artifact/org.hibernate/hibernate-entitymanager[Hibernate EntityManager]**

*** jest implementacją interfejsu JPA
 
** **http://mvnrepository.com/artifact/org.hibernate/hibernate-search-engine[Hibernate Search]**

***  dostarcza rozwiązań użycia indeksów Lucene

- rozwiązania alternatywne : http://lucene.apache.org/solr/[SOLR] , https://www.elastic.co/products/elasticsearch[ElasticSearch]


 
** **http://mvnrepository.com/artifact/org.hibernate/hibernate-validator[Hibernate Validator]**
...
*** np dla Neo4j : http://mvnrepository.com/artifact/org.hibernate.ogm/hibernate-ogm-neo4j[hibernate-ogm-neo4j]
*** np dla MongoDB : http://mvnrepository.com/artifact/org.hibernate.ogm/hibernate-ogm-mongodb [hibernate-ogm-mongodb]
 
** **DataSourcePool**
 
*** **http://mvnrepository.com/artifact/org.hibernate/hibernate-c3p0[hibernate-c3p0]** -   Integracja C3P0 z Hibernate  
...
*** http://mvnrepository.com/artifact/org.hibernate/hibernate-hikaricp[hibernate-hikaricp]  
*** http://mvnrepository.com/artifact/proxool/proxool[hibernate-proxool]   
 
** Cache

*** http://mvnrepository.com/artifact/org.hibernate/hibernate-ehcache[hibernate-ehcache]  
    
*** hibernate-infinispan 
 
 
== Podejścia alternatywne a stos Hibernate

image::standard_jpa_architecture.jpg[] 
...
Hibernate wspiera date i czas z javy 8 ale musimy podłączyć bibliotekę : http://mvnrepository.com/artifact/org.hibernate/hibernate-java8[hibernate-java8]  
...
-   http://proxool.sourceforge.net/[proxool]
   
-   http://www.jolbox.com/[boneCp]
   
-  https://commons.apache.org/proper/commons-dbcp/[Apache poll]
   
-  https://brettwooldridge.github.io/HikariCP/[hikarii]
...
-- unikanie niepotrzebnych połączeń (bazy i sterownika) po stosie TCP

-- zapobieganie usuwania tymczasowego bufora pamięci skojarzonego z danym połączeniem bazodanowym
...
=== Hibernate.cfg.xml jako deskryptor opisujący SessionFactory.

=== Przykład pliku hibernate.cfg.xml

CAUTION: lokalizacjia : classpath
...
<hibernate-configuration>
    <session-factory>       
        ...
        <property name="hibernate.connection.url">jdbc:h2:mem:testdbHibernate</property>
...
=== Hibernate.properties analogia do hibernate.cfg.xml 
...
hibernate.connection.url=jdbc:h2:mem:testdbHibernate
...
=== Wykorzystanie sessionBuilder'a
[source,java]
----
...
==== Przykład gotowego narzędzia do wytwarzania SessionFactory
...
=== Równość obiektów
 ...
 - Są równe jeśli operacja **_equals_** daje wynik pozytywny
...
== Nadawanie identyczności (czas, problem, rozwiązania)

NOTE: Patrz podrozdział annotation
...
-- Tańszy niż w przypadku odpytywania bazy

-- Gwarantowana unikalność nawet w przypadku różnych VM i nodów
 



***  **Sekwencja** - jeśli baza danych nie wspiera lub nie obsługuje wydajnie kluczy wygenerowanych przez UUID

*** **Własne** - w przypadku zorientowania na ekstremalną wydajność, **rozwiązanie własne**, aby nie odpytywać się bazy danych o następny numer sekwencji.
...
*** musi implementować metody **_equals_** i **_hasCode_**
...
** strona wielokrotna dwukierunkowego związku wiele do jednego nie może definiować **_mappedBy_** - jest zawsze 'posiadającą stroną relacji'
...
CAUTION: Stosujemy jeśli wymaga tego charakterystyka biznesu

NOTE: Złamanie enkapsulacji

NOTE: Często tworzone automatycznie gdy stosujemy inżynierie wsteczną - generatory !
...
WARNING: **Jaywalking** (przechodzenie na czerwonym świetle)
...
-- Długość pola przetrzymującego id's


NOTE: Denormalizacja

NOTE: Tabela łącząca wzorzec @ManyToMany

NOTE: Powiązane z Keyless-Entry
...
*** Na podstawie danego Id metoda **_get()_** próbuje pobrać obiekt z bazy danych. Jeśli obiekt nie istnieje zwraca null. Metoda **get()** w przeciwieństwie do metody load() uderza do bazy bezpośrednio.
...
NOTE: Pomaga uzyskać trwałość obiektom ulotnym i propagować je do bazy danych bez potrzeby wywołań zarządcy trwałości save()
...
Jeśli metoda remove jest wywołana na encji odłączonej zostanie wygenerowany wyjątek **_IllegalArgumentException_** lub zatwierdzenie transakcji się nie powiedzie.
Dane fizycznie zostaną usunięte w momencie zakończenia transakcji lub wykonania metody **_flush()_**
...
** Zmienia stan z odłączonego na zarządzany
...
WARNING:  Naive Trees (naiwne drzewa)

-- odnośnik tylko do swojego rodzica

-- **Path Enumeration** . Wprowadzenie ścieżki przejścia 'path'

[source,sql]
----
SELECT *
FROM Comments AS c
WHERE '1/4/5/11/' LIKE c.path || '%' ;
...
NOTE: Stosuj zapytania rekurencyje (Recursive Queries) WITH | CONNECT BY PRIOR
...
CREATE TABLE employee (
 ...
 SELECT employee_id, manager_id, first_name, last_name  FROM employee  START WITH employee_id = 1  CONNECT BY PRIOR employee_id = manager_id;
----
...
** **OptimisticLockException** - modyfikacja encji chronionej przez **_@Version_**
...
----
 ...
 - antipattern : czytelne hasła (Readable Passwords)
...
==  http://przewidywalna-java.blogspot.com/2014/09/querydsl-metamodel-generation-spring.html[Alternatywy QueryDSL]
